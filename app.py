...
class App(CTk):
    def __init__(self):
        super().__init__()
        self.is_menu = False
        self.menu_animation = -10
        self.geometry("500x400")
        self.title("Шарікі упали")
        """MENU"""
        self.menu_frame = CTkFrame(self, fg_color="#2cd835", width=30, height=400)
        ...
        """SCROLL FRAME"""
        self.scroll_frame = CTkScrollableFrame(self, fg_color="#36c12f")
    ...
    def show_menu(self):
        ...
        if self.is_menu and not self.menu_frame.winfo_width() >= 200:
            ...
            self.after(10, self.show_menu)
            if self.name_lbl and self.name_entry and self.btn_button:
    ...
    def toggle_menu(self):
        if self.is_menu:
            ...
            self.name_entry = CTkEntry( self.menu_frame, width=140, placeholder_text="Введіть ім'я")
            self.btn_button = CTkButton(self.menu_frame, text="Зберегти",fg_color="#36ff2f", command=self.save_name)
            self.name_lbl.pack(pady=40)
            self.name_entry.pack()
            self.btn_button.pack(pady=10)
    ...
    def save_name(self):
        if self.name_entry.get():
            ...
            try:
                ...
                self.sock.connect(("2.tcp.eu.ngrok.io", 19764))
                self.sock.send(self.name.encode())
                threading.Thread(target=self.recv_msgs(), daemon=True).start()
            except Exception as e:
                print(e)
                self.add_recv_msg("Сервер не знайдено ,плак-плак")
                self.sock.close()

    def adaptive_ui(self):
        self.menu_frame.configure(width=self.menu_frame.winfo_width())
        self.scroll_frame.place(x=self.menu_frame.winfo_width())
        self.scroll_frame.configure(width=self.winfo_width() - self.menu_frame.winfo_width(), height=self.winfo_height())
        self.msg_text_entry.place(x=self.winfo_width() - self.menu_frame.winfo_width(), y=self.winfo_height()- 50)
        self.msg_text_entry.configure(width=self.winfo_width() - self.menu_frame.winfo_width() - self.btn_send.winfo_width()- 15)

        self.msg_text_entry.place(x=self.menu_frame.winfo_width() + 5, y=self.winfo_height() - 50)
        self.btn_send.place(x=self.msg_text_entry.winfo_height() + self.menu_frame.winfo_height() + 10, y=self.winfo_height() - 50)


        self.after(50, self.adaptive_ui)

    def add_recv_msg(self, text):
        msg_frame = CTkFrame(self.scroll_frame, fg_color="white")
        msg_frame.pack(pady=10, anchor="w")
        wrap_width = self.winfo_width() - msg_frame.winfo_width() - 40
        msg_lbl = CTkLabel(msg_frame,text=text,wraplength=wrap_width,justify="left", fg_color="white")
        msg_lbl.pack(padx=10, pady=5)

    def add_send_msg(self, text):
        msg_frame = CTkFrame(self.scroll_frame, fg_color="white")
        msg_frame.pack(padx=10, anchor="e")
        wrap_width = self.winfo_width() - msg_frame.winfo_width() - 40
        msg_lbl = CTkLabel(msg_frame, text="Я  " + text, wraplength=wrap_width, justify="left", fg_color="white")
        msg_lbl.pack(padx=10, pady=5)
